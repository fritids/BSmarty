...
function mysqltoxml () {
	...
	$chemin = "/var/www/vhosts/atalante-pathologie.com/httpdocs/XML_Infologic/";
	$numerolabo = "010";
	...
	if ($num_rows != 0) {
		...
		if ($num_rows != 0) {
			...
			do {
				...
				/* Ajout des champs pour la V2*/
				$CodeCaisse = utf8_encode($row_drw['CodeCaisse']);
				$FinValiditeDroit = utf8_encode($row_drw['FinValiditeDroit']);
				$RangGemelaire = utf8_encode($row_drw['RangGemelaire']);
				...
				/* Ajout des champs XML pour la V2 */
				$prop = $dom->createElement('CodeCaisse',$CodeCaisse);
				$book->appendChild($prop);
				$prop = $dom->createElement('FinValiditeDroit',$FinValiditeDroit);
				$book->appendChild($prop);
				$prop = $dom->createElement('RangGemelaire',$RangGemelaire);
				$book->appendChild($prop);
